...
- **Intelligent Viewport Management**:
    - **Interaction Debouncing**: Delay expensive `UpdateViewport` calls by ~500ms after interaction stops. This keeps the pan/zoom experience smooth by using cached tiles.
    - **Exhaustion Detection**: If the user moves beyond a threshold (e.g., > 0.5 tiles or > 0.5 zoom levels), force an immediate update to prevent empty space.
